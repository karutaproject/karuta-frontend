...
//==================================
{
	...
	html += "		<div class='col-1'>";
	//------------ buttons ---------------
	html += UIFactory.UsersFolder.getAdminFolderMenu(usersfolder,false);
	//---------------------------------------
	html += "		</div><!-- class='col-1' -->";
	html += "		<div class='col-1'>";
	//------------------------ menu-burger
...
//==================================
{
	...
	for (var i=0; i<list.length;i++){
		...
		if("USERSFOLDER"==list[i]['type']) {
			html += "<div class='row item item-user' id='item-usersfolder_"+item.id+"'>";
			html += item.getView("#item-usersfolder_"+item.id,type,langcode);
			html += "</div>";
		} else {
			var itemid = item.id;
			html += "<div class='row item item-folder' id='item-user_"+itemid+"'>";
			html += item.getView("#item-user_"+itemid,'list2',langcode);
			html += "</div>";
		}
	}
	$("#"+dest).html($(html));
...
//==================================
{
	...
	if (type=='list') {
		...
		if (USER.admin || (this.owner=='Y') || (USER.creator && !USER.limited)) {
			html += UIFactory.UsersFolder.getAdminFolderMenu(this,true);
		}
		html += "</div><!-- class='col' -->";
		//------------------------------------
	}
...
//======================
UIFactory["UsersFolder"].getAdminFolderMenu = function(self,list)
//======================
{	
	var html = "";
	html += "<div class='dropdown folder-menu'>";
...
//----------------------------------------------------------------------------------
//==================================
function loadAndDisplayActiveUsersFolder(dest,id,langcode) {
//==================================
	$("#wait-window").show();
