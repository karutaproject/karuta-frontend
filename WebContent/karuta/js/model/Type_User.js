...
//==================================
{
	...
	if (type=='list0') {
		...
		if (USER.admin){
			...
			//----------------------------------
			html += "<button class='btn ' onclick=\"UIFactory.UsersGroup.displayGroupsByUser('"+this.id+"',true)\"";
			...
			if (this.username_node.text()!='root') {
				html += "<button class='btn ' onclick=\"UIFactory.Portfolio.displayUserPortfolios('"+this.id+"','"+this.firstname+"','"+this.lastname+"',false,true)\">";
...
//==============================
{
	...
	$.ajax({
		...
		success : function(data) {
			...
			} else if (type=='select-owner'){
				$("#"+type+"-rightside-users-content1").html("");
				for (var i=0; i<searched_active_users_list.length;i++){
					$("#"+type+"-rightside-users-content1").append(searched_active_users_list[i].getRadio(null,null,'select_users'));
					$("#"+type+"-rightside-users-content1").append("<br>");
				}				
			} else {
				$("#"+type+"-rightside-content1").show();
...
{	
	...
	if (type!='list3' && type!="list-ondrop") {
		//----------------------------------
		html += "<span class='button btn' onclick=\"UIFactory['UsersGroup'].displayGroupsByUser('"+this.id+"',true)\"";
		if (this.username_node.text()!='root' && this.username_node.text()!='public') {
			html += ">";
		} else {
			html += " disabled='true'>";
		...
		//----------------------------------
		if (this.username_node.text()!='root') {
			html += "<span class='button btn' onclick=\"UIFactory.Portfolio.displayUserPortfolios('"+this.id+"','"+this.firstname+"','"+this.lastname+"',false,true)\">";
